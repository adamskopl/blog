...
This post aims to present an idea for the "Multiscreen Content Display"
software project.

Resulting software and its source code will be available publicly. Ongoing work
reports will be published on this blog.

## Goal

The goal of the MCD is to allow a user to display content's (e.g. an image or
external webpage) chosen areas in multiple web browsers run on different
devices. The devices' physical positions and sizes are taken into account. The setup
...
User will be able to display the menu's different sections on different devices'
browsers. For example tablet with a running browser could display "VEGE BURGERS"
section and smartphones could do the same with other menu areas.
...
* arrange displays (what area should a particular display show?)
* scale displays (zooming displays in/out)

Adjusting devices' properties (position, scale) relative to the content, will be
also possible through each browser involved. In that way, the user will be allowed
to create the desired setup by interacting both with an editor and target devices.
...
1. Using the editor, the user creates a new display project. Project's web link is
   created.
2. Content is chosen. It could be an image or a web page.
3. The user opens the project's web link using target devices' browsers. At this stage
...
4. Devices' display areas are visualized in the editor. User modify their
   positions and size relative to the content, so that physical devices will
   display desired areas of the content.
5. After the setup, the editor or browser of any device can be closed and opened again. The once created content and devices setup will be restored for every device.
...
but thankfully only once for each device (the software will store calibration
settings).

### Synchronization

Synchronization between the editor and the devices will be needed. Examples:
- Different content is chosen: devices should display it.
- The position of a device is changed in the editor: the device should display a
  different fragment.

As the project is web-based, bidirectional communication between browsers and
the server is needed.

## Summary

"Multiscreen Content Display" will allow displaying a chosen content using
multiple devices. With the ability to choose positions of content's fragments
